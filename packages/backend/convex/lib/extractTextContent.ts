...
const SYSTEM_PROMPT = {
    image: "You turn images to text. If it is a photo of a document, transcribe it. If it is not a document, describe it.",
...
export async function extractTextContent(
    ...
    if (SUPPORTED_IMAGE_TYPES.some((type) => type === mimeType)) {
        return extractTextImage(url);
    };

    if (mimeType.toLowerCase().includes("pdf")) {
        return extractTextPdf(url, mimeType, filename);
    }
...
async function extractTextImage(url: string): Promise<string> {
    const result = await generateText({
